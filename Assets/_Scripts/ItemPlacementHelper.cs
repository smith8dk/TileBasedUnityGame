...
public class ItemPlacementHelper
{
    ...
    public Vector2? GetItemPlacementPosition(PlacementType placementType, int iterationsMax, Vector2Int size, bool addOffset)
    {
        ...
        var candidates = tileByType[placementType];
        if (candidates.Count < itemArea)
            return null;
        ...
        {
            iteration++;
            var position = candidates.ElementAt(UnityEngine.Random.Range(0, candidates.Count));
            if (itemArea > 1)
            {
                var (ok, placements) = PlaceBigItem(position, size, addOffset);
                if (!ok) continue;
                candidates.ExceptWith(placements);
                if (tileByType.ContainsKey(PlacementType.NearWall))
                    tileByType[PlacementType.NearWall].ExceptWith(placements);
            }
            else
            {
